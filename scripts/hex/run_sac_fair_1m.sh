...
cd "${REPO_ROOT}"
RUN_NAME="${HEX_RUN_NAME:-sac_fair_six_sequence_seed13_1m_$(date -u +%Y%m%dT%H%M%SZ)}"
...
PY
python -u scripts/train_general_one_hand_policy.py --timesteps 1000000 --seed 13 --lookahead 1 --curriculum sequence_cleanup --sequence-pitches '73;74;75;73,75;75,73;74,75' --sequence-sampling-weights '0.22,0.22,0.22,0.14,0.10,0.10' --stage-name sac_fair_six_sequence_seed13_1m --action-mode direct --action-repeat 1 --reward-profile transition_cleanup --checkpoint-freq 100000 --sequence-timing-profile aligned --output-dir /workspace/runs/${RUN_NAME}/output 2>&1 | tee /workspace/runs/${RUN_NAME}/logs/train.log"
